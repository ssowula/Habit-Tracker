...
function renderHabitsList(habits) {
    ...
    if (habits.length > 0) {
        habits.forEach(habit => {
            ...
            if (isCompleted) {
                listItem.classList.add('done', 'completed-permanently');
            }
            listItem.innerHTML = `
                <input type="checkbox" id="habit-${habit.id}" class="today_checkbox" ${isCompleted ? 'checked disabled' : ''}>
        ...
        try {
            ...
            if (allOk) {
                ...
                checkedCheckboxes.forEach(checkbox => {
                    ...
                    checkbox.closest('li').classList.add('completed-permanently');
                });
                updateCompleteButtonVisibility();
            } else {
    ...
    if (pagePath.endsWith('/index.html') || pagePath.endsWith('/')) {
        ...
        if (token) {
            try {
                ...
                console.error("Błąd weryfikacji tokenu przy starcie:", error);
            }
        }
    }
